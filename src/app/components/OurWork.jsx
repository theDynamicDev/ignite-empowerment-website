...
const OurWork = () => {
  ...
  return (
   ...
   className="flex flex-col content w-full flex-wrap md:flex-row md:flex-wrap md:justify-evenly items-center  justify-center md:items-start -z-40 md:z-30 bottom-12 ">
   <Text
              
                css={{
                  textGradient: "45deg, $red700 -70%, $yellow600 30%, $green600 60%, $cyan500 100%",
                }}
                weight="bold"className=" relative flex top-10 md:top-20 text-2xl md:flex-col -z-40 flex text-center bottom-20 md:bottom-0  md:text-5xl text-white items-start tracking-wide leading-10 indent-2 drop-shadow-xl"
               >
      ...
      animate={controls}
        ...
        >
          <Image
           ...
           fill
            center
            className={`lg:object-cover flex content-center  ${isHovered ? "transition-transform duration-500 ease-out-in" : "opacity-100"}` }  onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}
          />
         <div className={`absolute inset-0 ${image.color} opacity-60`} />
          <div className={`absolute inset-0 flex items-center justify-center `}>
            <p className="text-white text-2xl lg:text-lg font-bold tracking-widest  transition-transform duration-500 ease-in-out hover:scale-x-125 hover:scale-y-125 text-shadow-lg ">{image.text}</p>
          </div>
